import { loadTrainerOnboardingState } from '../_lib/state';

// Stub — Agent F replaces this with the 3-tab Application form.
export default async function OnboardingApplicationPage() {
  const state = await loadTrainerOnboardingState();
  return (
    <div className="rounded-[1.25rem] border border-[#41627B]/20 bg-white p-8 text-[#173041] shadow-[0_18px_44px_rgba(45,79,103,0.08)]">
      <p className="text-xs font-bold uppercase tracking-[0.2em] text-[#2D4F67]/56">Step 1 — Application</p>
      <h2 className="mt-3 text-2xl font-black tracking-tight">Coming soon</h2>
      <p className="mt-2 text-sm text-[#2D4F67]/72">
        The Contact, Qualifications, and Sales Goals tabs are landing in the next pass.
      </p>
      <p className="mt-4 text-xs text-[#2D4F67]/56">trainer: {state.trainerName}</p>
    </div>
  );
}
