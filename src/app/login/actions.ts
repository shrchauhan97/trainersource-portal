...
export async function createPasswordFirstTimeAction(
  rawEmail: string,
  rawPassword: string
  ...
  const { data: trainer } = await supabaseService
    .from('trainers')
    .select('id, status')
    .eq('email', email)
    .maybeSingle();
  ...
  if (!admin && !trainer) {
    return { ok: false, error: 'Email not found in system.' };
  }
  ...
  const { data: authUsersData, error: listError } = await supabaseService.auth.admin.listUsers();
  
  if (listError || !authUsersData?.users) {
    console.error('[createPasswordFirstTime] listUsers failed', listError);
    return { ok: false, error: 'Failed to set password. Please try again.' };
  }

  let authUser = authUsersData.users.find(u => u.email === email);
  
  // If auth user doesn't exist yet, create it
  if (!authUser) {
    ...
    if (createError || !newUser?.user?.id) {
      console.error('[createPasswordFirstTime] createUser failed', createError);
      return { ok: false, error: 'Failed to set up account. Please try again.' };
    }
  ...
  if (error) {
    console.error('[createPasswordFirstTime] updateUser failed', error);
    return { ok: false, error: 'Failed to set password. Please try again.' };
  }

  // Get role and determine next URL
  const role = admin ? 'admin' : 'trainer';
